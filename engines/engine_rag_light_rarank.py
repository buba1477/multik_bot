...
_QA_PROMPT_STR = (
    ...
    "6. ТЕСТЫ: Если в вопросе явно перечислены варианты ответов (А/Б/В/Г или 1/2/3/4) для выбора одного — выбери самый точный по контексту. При вопросах-перечислениях используй все релевантные фрагменты контекста.\n"
    ...
    "10. ПЕРЕЧНИ (полнота ответа): Если вопрос просит перечислить виды, категории, основания, документы, меры ответственности, взыскания или иной полный перечень — проанализируй ВЕСЬ предоставленный контекст. Собери элементы из ВСЕХ релевантных норм, удали дубли и сформируй итоговый перечень. НЕ используй «и другие», «и т.д.» вместо конкретных элементов, если они присутствуют в контексте.\n    11. ПРОВЕРКА ПОЛНОТЫ (СТРОГО): Ты ОБЯЗАН выполнить проверку своего ответа перед отправкой. Выпиши все элементы перечня из КАЖДОГО чанка контекста. Если хотя бы один чанк содержит элемент, которого нет в твоём ответе — ты ОБЯЗАН включить его. Если в разных чанках приведён перечень разного объёма — используй НАИБОЛЕЕ ПОЛНЫЙ из имеющихся перечней. Категорически ЗАПРЕЩЕНО отвечать только по одному чанку, если другие чанки содержат дополнительные элементы.\n\n"
    "ВОПРОС: {query_str}\n\n"
    "ОТВЕТ ЭКСПЕРТА:"
)
...
class RerankedEngine:
    def __init__(self, index: Any, qa_prompt: Any, initial_top_k: int = 30, final_top_k: int = 5):
        ...
        # ========== РЕРАНКЕР (PyTorch) ==========
        try:
            # Путь к папке с PyTorch-моделью
            reranker_path = os.path.join(os.path.dirname(__file__), "reranker_m3")
            
            # Проверяем, есть ли папка, если нет — пробуем альтернативный путь
            if not os.path.exists(reranker_path):
                reranker_path = os.path.join(os.path.dirname(__file__), "GraphRAG/markdown/reranker_m3")
            
            logger.info(f"🧠 Загрузка PyTorch реранкера из {reranker_path}...")
            ...
            self.reranker.eval()
            logger.info("✅ PyTorch реранкер загружен")
            self.use_pytorch_reranker = True
        except Exception as e:
            logger.error(f"⚠️ Реранкер не загрузился: {e}")
            self.reranker = None
            self.use_pytorch_reranker = False
    ...
    def _sync_query(self, query_text: str):
        ...
        # ВЫВОД ТОП-10 ГИБРИДА
        logger.info("="*20 + " HYBRID TOP-10 " + "="*20)
        for i, n in enumerate(initial_nodes[:10]):
            nid = n.node.metadata.get('id', 'N/A')
            logger.info(f"Rank {i+1:2}: [{n.score:.4f}] ID: {nid}")
        logger.info("="*55)
        ...
        if self.reranker and initial_nodes:
            logger.info("🧪 Запуск реранкинга (10 нод)...")
            rt_start = time.time()
            
            # СТАВИМ СТРОГО 10 НОД
            nodes_to_rerank = initial_nodes[:10]
            passages = [n.node.get_content() for n in nodes_to_rerank]
            ...
            for node, score in zip(nodes_to_rerank, scores):
                node.score = float(score)
            
            # Сортируем то, что реранкнули, и берем final_top_k (обычно 5)
            final_nodes = sorted(nodes_to_rerank, key=lambda x: x.score, reverse=True)[:self.final_top_k]
            logger.info(f"⏱ Реранкинг занял {time.time() - rt_start:.2f} сек")
        else:
            final_nodes = initial_nodes[:self.final_top_k]
            logger.info("⚠️ Реранкер пропущен")

        # ВЫВОД ФИНАЛЬНОГО ТОП-5 ДЛЯ МОДЕЛИ
        logger.info("!"*20 + " FINAL TOP-5 FOR LLM " + "!"*20)
        for i, n in enumerate(final_nodes):
            nid = n.node.metadata.get('id', 'N/A')
            words = len(n.node.get_content().split())
            logger.info(f"TOP-{i+1:2}: [{n.score:7.4f}] ID: {nid} (~{words} слов)")
        logger.info("!"*61)
        ...
        mode = ResponseMode.COMPACT if is_visual else ResponseMode.TREE_SUMMARIZE
        logger.info(f"🚀 РЕЖИМ: {mode}")
        
        synth = get_response_synthesizer(text_qa_template=self.qa_prompt, streaming=True, response_mode=mode)
        
        # 4. Генерация
        final_nodes.sort(key=lambda x: x.score, reverse=True)
        forced_query = f"{query_text}\n\nВАЖНО: ОТВЕТЬ НА РУССКОМ ЯЗЫКЕ, ИСПОЛЬЗУЯ ТОЛЬКО БАЗУ ЗНАНИЙ. Пиши дроби как 1/4 (БЕЗ LaTeX)."
        
        logger.info("🚀 Отправка в Ollama...")
        response = synth.synthesize(forced_query, final_nodes)
        
        logger.info(f"🏁 [DONE] Запрос выполнен за {time.time() - overall_start:.2f} сек")
        return response
