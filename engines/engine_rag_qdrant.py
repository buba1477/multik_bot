...
class RerankedEngine:
    def __init__(self, index: Any, qa_prompt: Any, initial_top_k: int = 30, final_top_k: int = 5):
        # Сначала инициализируем инструменты
        ...
        # Теперь запускаем тяжелую логику (теперь _tokenize точно виден)
        try:
            logger.info("🔧 Инициализация гибридного движка...")
            # ВАЖНО: Если используешь Qdrant, index.docstore.docs может быть пустым.
            # Если нод 0, BM25 просто не соберется, но бот не упадет.
            ...
            if self.all_nodes:
                ...
                logger.warning("⚠️ База знаний пуста (docstore пуст). BM25 пропущен.")
        ...
        try:
            ...
            # Раскомментируй, если нужен реранкер
            self.reranker = SentenceTransformerRerank(model=reranker_path, top_n=self.final_top_k, device="cpu")
            #self.reranker = None   
    ...
    def _sync_query(self, query_text: str):
        ...
        if self.reranker and initial_nodes:
            logger.info("🧪 Запуск реранкинга BGE-M3 (CPU)...")
            rt_start = time.time()
            # Берем топ-10 после гибрида и пересчитываем их
            ...
            # Если реранкер выключен, просто берем топ из гибрида
            final_nodes = initial_nodes[:self.final_top_k]
            logger.info("⚠️ Реранкер пропущен (использую топ гибрида)")

        # 🔥 ЛОГИ ПОСЛЕ РЕРАНКЕРА
        print("\n" + "!"*20 + " FINAL TOP ДЛЯ LLM " + "!"*20)
        for i, n in enumerate(final_nodes):
            nid = n.node.metadata.get('id', 'N/A')
            print(f"TOP-{i+1}: ID: {nid}")
        print("!"*61 + "\n")

        # 4. ГЕНЕРАЦИЯ ОТВЕТА
        current_mode = ResponseMode.SIMPLE_SUMMARIZE 
        ...
        logger.info(f"🏁 [DONE] Время: {time.time() - overall_start:.2f} сек")
        return response
